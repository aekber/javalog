var ratingsL10n = {
		...
		ratings_mouseover_image.src='/javalog-web/resources/img/rating_over.png';
		...
		function ratings_off(b) {		
			if (!is_being_rated) {
				for (i = 1; i <= ratingsL10n.max; i++) {
					if (i <= b) {						
						jQuery('#rating_' + post_id + '_' + i).attr('src', '/javalog-web/resources/img/rating_on.png');
					} else {					
						if (i <= (b + 0.5)) {
							jQuery('#rating_' + post_id + '_' + i).attr('src', '/javalog-web/resources/img/rating_half.png');							
						} else {
							jQuery('#rating_' + post_id + '_' + i).attr('src','/javalog-web/resources/img/rating_off.png');
						}
					}
				}
				if (jQuery('#ratings_' + post_id + '_text').length) {
