...
var imagesPath = "/javalog-web/resources/img/";

// Initialization function - if you have any other 'onload' functions, add them
// here
function init() {
...
function preloadImages() {
	...
	preloads[0].src = imagesPath + "button_left_xon.gif";
	preloads[1] = new Image();
	preloads[1].src = imagesPath + "button_right_xon.gif";
	preloads[2] = new Image();
	preloads[2].src = imagesPath + "input_left_xon.gif";
	preloads[3] = new Image();
	preloads[3].src = imagesPath + "input_right_xon.gif";
	preloads[4] = new Image();
	preloads[4].src = imagesPath + "txtarea_bl_xon.gif";
	preloads[5] = new Image();
	preloads[5].src = imagesPath + "txtarea_br_xon.gif";
	preloads[6] = new Image();
	preloads[6].src = imagesPath + "txtarea_cntr_xon.gif";
	preloads[7] = new Image();
	preloads[7].src = imagesPath + "txtarea_l_xon.gif";
	preloads[8] = new Image();
	preloads[8].src = imagesPath + "txtarea_tl_xon.gif";
	preloads[9] = new Image();
	preloads[9].src = imagesPath + "txtarea_tr_xon.gif";
}
// getting all the required elements
function getElements() {
	
	var eform = document.getElementById('third');
	var a = eform.getElementsByTagName('input').length;
	var b = eform.getElementsByTagName('label').length;
	var c = eform.getElementsByTagName('textarea').length;

			for ( var nfi = 0; nfi < a; nfi++) {
				inputs.push(eform.getElementsByTagName('input')[nfi]);
			}
			for ( var nfl = 0; nfl < b; nfl++) {
				labels.push(eform.getElementsByTagName('label')[nfl]);
			}
			for ( var nft = 0; nft < c; nft++) {
				textareas.push(eform.getElementsByTagName('textarea')[nft]);
			}
...
function replaceTextareas() {
	for ( var q = 0; q < textareas.length; q++) {
		...
		textareas[q].onfocus = function() {
			...
			this.parentNode.nextSibling.getElementsByTagName("img")[0].src = imagesPath
					+ "txtarea_bl_xon.gif";
		};
...
function buttonHovers() {
	for ( var i = 0; i < buttons.length; i++) {
		...
		if (buttons[i].nextSibling) {
			buttons[i].parentNode.insertBefore(buttonRight,
					buttons[i].nextSibling);
		} else {
